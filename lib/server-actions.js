...
import { GetAiResponse } from "./ai-functions";
import { LessonsSchema } from "./z-schemas";
...
export const CreateBook = async ({ data }) => {
  ...
  const prompt = `${book.contentPrompt}. Τίτλος: ${book.title}. Περιγραφή: ${JSON.stringify(book.description)}. Κατηγορία: ${book.category}. `;

  const response = await GetAiResponse({ prompt, schema: LessonsSchema });

  book.lessons = response.lessons;
  await book.save();
